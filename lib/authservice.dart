...
class AuthService {
  ...
  Stream<User> get user {
    ...
    .map(_userFromFireBaseUser); //map out the stream and get the properties of the user from the database and passes it to the user object 
  }
  ...
  Future signUp(email, password) async {
     try{ // try to create the user
      AuthResult credentials = await _auth.createUserWithEmailAndPassword(email:email, password: password);
      FirebaseUser user = credentials.user;
      updateProfile(user);
      return _userFromFireBaseUser(user); // call the Update profile so we can inject the data on database
